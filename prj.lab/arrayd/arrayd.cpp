...
ArrayD::ArrayD(const std::ptrdiff_t size)
    ...
    , size_(size) {
    if (size_ <= 0) {
        throw std::invalid_argument("ArrayD::ArrayD - size must be positive");
    }
...
void ArrayD::Resize(const std::ptrdiff_t size) {
    if (size < 0) {
        throw std::invalid_argument("ArrayD::Resize - size must be positive");
    }
    if (capacity_ < size) {
...
double& ArrayD::operator[](const std::ptrdiff_t idx) {
    if (idx < 0 || size_ <= idx) {
        throw std::invalid_argument("ArrayD::operator[] - index out of range");
    }
    return *(data_ + idx);
}
...
void ArrayD::Insert(const std::ptrdiff_t idx, const double val) {
    if (idx < 0 || size_ < idx) {
        throw std::invalid_argument("ArrayD::Insert - index out of range");
    }
    Resize(size_ + 1);
    if (idx != Size() - 1) {
